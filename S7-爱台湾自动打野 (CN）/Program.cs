...
namespace AutoJungle
{
    internal class Program
    {
        ...
        {
            menu = new Menu("爱台湾S7-自动打野", "AutoJungle", true);

            Menu menuD = new Menu("调试 ", "dsettings");
            menuD.AddItem(new MenuItem("debug", "控制台显示调试信息"))
                .SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press))
                .SetFontStyle(FontStyle.Bold, SharpDX.Color.Orange);
            menuD.AddItem(new MenuItem("State", "显示游戏状态")).SetValue(false);
            menu.AddSubMenu(menuD);
            Menu menuJ = new Menu("野区设置", "jsettings");
            menuJ.AddItem(new MenuItem("HealtToBack", "回城血量(%)").SetValue(new Slider(35, 0, 100)));
            menuJ.AddItem(new MenuItem("UseTrinket", "使用饰品")).SetValue(true);
            menuJ.AddItem(new MenuItem("EnemyJungle", "入侵敌方野区")).SetValue(true);
            menu.AddSubMenu(menuJ);
            Menu menuG = new Menu("Gank设置", "gsettings");
            menuG.AddItem(new MenuItem("GankLevel", "最低等级进行Gank").SetValue(new Slider(5, 1, 18)));
            menuG.AddItem(new MenuItem("GankFrequency", "Gank次数").SetValue(new Slider(100, 0, 100)));
            menuG.AddItem(new MenuItem("GankRange", "搜索范围").SetValue(new Slider(7000, 0, 20000)));
            menuG.AddItem(new MenuItem("ComboSmite", "使用惩戒")).SetValue(true);
            menu.AddSubMenu(menuG);
            menu.AddItem(new MenuItem("Enabled", "开启挂机")).SetValue(true);
            menu.AddItem(new MenuItem("AutoClose", "游戏结束自动关闭")).SetValue(true);
            Menu menuChamps = new Menu("支持的英雄", "supported");
            menuChamps.AddItem(new MenuItem("supportedYi", "易大师"));
            menuChamps.AddItem(new MenuItem("supportedWarwick", "狼人"));
            menuChamps.AddItem(new MenuItem("supportedShyvana", "龙女"));
            ...
            menu.AddItem(
            new MenuItem("AutoJungle2", "合成血手/战士后F5，挂机则会无法使用!")
